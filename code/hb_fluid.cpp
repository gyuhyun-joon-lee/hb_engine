...
initialize_fluid_cube(FluidCube *fluid_cube, MemoryArena *arena, u32 cell_count_x, u32 cell_count_y, u32 cell_count_z, f32 cell_dim, f32 viscosity)
{
    ...
    fluid_cube->stride = sizeof(f32)*total_cell_count;
    fluid_cube->v_x_source = fluid_cube->v_x;
    fluid_cube->v_x_dest = fluid_cube->v_x_source + fluid_cube->stride;
    fluid_cube->v_y_source = fluid_cube->v_y;
    fluid_cube->v_y_dest = fluid_cube->v_y_source + fluid_cube->stride;
    fluid_cube->v_z_source = fluid_cube->v_z;
    fluid_cube->v_z_dest = fluid_cube->v_z_source + fluid_cube->stride;

    fluid_cube->density_source = fluid_cube->densities;
    fluid_cube->density_dest = fluid_cube->density_source + fluid_cube->stride;
}
            ...
            ++cell_z)
    {
                ...
                ++cell_y)
        {
                    ...
                    ++cell_x)
            {
                ...
                dest[get_index(cell_count, cell_x, cell_y, cell_z)] = force[ID] + dt*source[ID]; 
            }
        }
    }
}
            ...
            ++cell_z)
    {
                ...
                ++cell_y)
        {
                    ...
                    ++cell_x)
            {
                ...
                v3 p = V3(cell_x, cell_y, cell_z) - dt*cell_based_u;
                ...
                else if(p.x > cell_count.x + 0.5f)
                {
                    p.x = cell_count.x + 0.5f;
                }
                ...
                else if(p.y > cell_count.y + 0.5f)
                {
                    p.y = cell_count.y + 0.5f;
                }
                ...
                else if(p.z > cell_count.z + 0.5f)
                {
                    p.z = cell_count.z + 0.5f;
                }
...
project(f32 *x, f32 *y, f32 *z, f32 *pressures, v3u cell_count, f32 cell_dim, f32 viscosity, f32 dt)
{
    ...
    set_boundary_values(x, cell_count, ElementTypeForBoundary_x);
    set_boundary_values(y, cell_count, ElementTypeForBoundary_y);
    set_boundary_values(z, cell_count, ElementTypeForBoundary_z);
}
