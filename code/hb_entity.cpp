...
internal Entity *
add_cube_rigid_body_entity(GameState *game_state, v3 p, v3 half_dim, f32 inv_mass, v3 color)
{
    ...
    {
        // TODO(joon) I dont like this double conversion...
        f32 mass = 1.0f/inv_mass;
        inertia_tensor = M3x3(mass*(dim.y*dim.y + dim.z*dim.z)/12.0f, 0, 0,
                              0, mass*(dim.x*dim.x + dim.z*dim.z)/12.0f, 0,
                              0, 0, mass*(dim.x*dim.x + dim.y*dim.y)/12.0f);
    }

    result->rb = init_rigid_body(p, inv_mass, inertia_tensor);
