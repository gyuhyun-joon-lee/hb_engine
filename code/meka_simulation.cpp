...
move_mass_agg_entity(GameState *game_state, Entity *entity, f32 dt_per_frame, b32 magic)
{
            ...
            ++connection_index)
    {
        ...
        if(connection->particle_index_0 == 4 || connection->particle_index_1 == 4)
        {
            int breakhere = 0;
        }

        MassParticle *particle_0 = mass_agg->particles + connection->particle_index_0;
        MassParticle *particle_1 = mass_agg->particles + connection->particle_index_1;
        ...
        f32 connection_rest_length_threshold = 0.01f * connection->rest_length;
        if(abs(length_diff) >= connection_rest_length_threshold)
        {
            ...
            ++particle_index)
    {
        ...
        if(is_entity_flag_set(entity, Entity_Flag_Collides))
        {
            ...
            {
                // NOTE(joon) we did not hit anything
            }

            f32 min_t_epsilon = 0.01f;
            // TODO(joon) avoid epsilon?
            if(min_t > min_t_epsilon)
            {
                particle->p += (min_t - min_t_epsilon) * dp;
            }
        }
        else
        {
